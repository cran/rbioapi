...
if (is.data.frame(enrichr_enrich) && hasName(enrichr_enrich, "Adjusted.P.value")) {
  ...
  enrichr_enrich <- enrichr_enrich[1:min(10, nrow(enrichr_enrich)),]
...
if (utils::hasName(reactome, "pathways") && is.data.frame(reactome$pathways)) {
  reactome$pathways <- reactome$pathways[1:min(10, nrow(reactome$pathways)),]
  
  DT::datatable(
    data = reactome$pathways,
    autoHideNavigation = TRUE,
    options = list(
      scrollX = TRUE, 
      paging = TRUE,
...
## ----reactome_summary, echo=TRUE, message=TRUE, eval=exists("reactome")&&hasName(reactome, "summary")----
...
if (utils::hasName(panther_enrich, "result") && is.data.frame(panther_enrich$result)) {
  panther_enrich$result <- panther_enrich$result[1:min(10, nrow(panther_enrich$result)),]
  
  DT::datatable(
    data = panther_enrich$result,
    autoHideNavigation = TRUE,
    options = list(
      scrollX = TRUE, 
      paging = TRUE,
...
if (utils::hasName(string_enrich, "PMID") && is.data.frame(string_enrich$PMID)) {
  ...
  string_enrich$PMID <- string_enrich$PMID[1:min(10, nrow(string_enrich$PMID)),]
  
  DT::datatable(
    data = string_enrich$PMID,
    autoHideNavigation = TRUE,
    options = list(
      scrollX = TRUE, 
      paging = TRUE,
...
if (utils::hasName(string_annot, "PMID") && is.data.frame(string_annot$PMID)) {
  string_annot$PMID <- string_annot$PMID[1:min(10, nrow(string_annot$PMID)),]
  
  DT::datatable(
    data = string_annot$PMID,
    autoHideNavigation = TRUE,
    options = list(
      scrollX = TRUE, 
      paging = TRUE,
...
## ----string_enrichment_plot, fig.show='hide'----------------------------------
...
## ----string_enrichment_plot_image, echo=FALSE, fig.cap="Visualization of enrichment analysis results", fig.align='center', fig.width=7----
