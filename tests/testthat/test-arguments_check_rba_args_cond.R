test_that(".rba_args_cond works", {
  ...
  expect_error(
    object = .rba_args_cond_nested(cond_i = list(TRUE))
  )

  expect_error(
    object = .rba_args_cond_nested(
      cond_i = list(quote(x < y), "sfsdf", TRUE, "dsf")
    )
  )

  expect_list_classes(
    obj = .rba_args_cond_nested(cond_i = list(quote(x < y))),
    classes = c("character", "logical")
  )
  expect_list_classes(
    obj = .rba_args_cond_nested(cond_i = list("x < y")),
    classes = c("character", "logical")
  )
  ...
  cond_i_l2_msg <- .rba_args_cond_nested(
    cond_i = list(quote(x < y), "error_msg")
  )
  ...
  cond_i_l2_warn <- .rba_args_cond_nested(cond_i = list(quote(x < y), TRUE))
  ...
  cond_i_l3 <- .rba_args_cond_nested(cond_i = list(quote(x < y), "error_msg", TRUE))
